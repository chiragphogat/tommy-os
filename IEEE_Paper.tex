...
\title{T.O.M.M.Y.: A Multi-Process Architecture for Hands-Free Windows Navigation Without Cloud Dependencies}
...
\begin{abstract}
Even now, computers keep us at our desks with keyboards and mice. Voice assistants are available, but if you ask one to drag and drop a file, it will not work. We made T.O.M.M.Y. to fix this. It is a hands-free wrapper that uses a regular webcam and microphone to take over Windows. But getting there was not easy. When I tried to run heavy vision models and a talking AI at the same time in Python, the GIL wall hit right away, and the screen froze every time the bot spoke. Also, standard webcams make it hard to judge depth. The camera thinks your hands got smaller when you lean back in your chair, which messes up the pinch-to-click math. And do not even get us started on trying to change the brightness of the screen in Windows. That completely stopped the main thread. We tore down the architecture to get around all of this. We divided everything into separate background processes that talked to each other through a small JSON file. We also set a hard limit of 55 pixels on the size of our click-hitboxes to fix the camera depth and put the clunky WMI hardware calls into separate daemons. To further counteract organic tendon jitter, we developed a Magnetic Cursor Lock that forcefully freezes positional tracking the moment a pinch boundary crosses 60 pixels, alongside a Time-Gated Ocular Engine that converts simple blinks into variable OS macros dictated purely by millisecond holds. The final build runs perfectly at 60 frames per second natively, and almost every action happens in less than 20 milliseconds.
\end{abstract}

\begin{IEEEkeywords}
Multimodal Interaction, Gesture Recognition, Inter-Process Communication, Automation, Large Language Models, MediaPipe, and Human-Computer Interaction.
\end{IEEEkeywords}
...
Think about how we really use computers today. The basic hardware format has not changed much since the 1980s. We still use physical mice on mousepads \cite{b1}. Big tech companies pushed voice tools \cite{b19}, but to be honest, they are mostly just new things. You ask them something, they send a message to a server, and maybe they play a song \cite{b12}. They don't really know what is going on on your screen, and they cannot really replace a mouse for work.

You can find a lot of cool prototypes that do hand tracking by looking around GitHub and reading recent papers. But we saw a big, obvious flaw: they do not work with real operating systems. If you try to combine an OpenCV vision loop and an offline text-to-speech engine into one Python script, the performance drops significantly \cite{b9}. The Global Interpreter Lock (GIL) in Python limits bytecode execution to one core \cite{b5}. So, in practice, the moment your voice assistant starts talking, your mouse cursor stops moving on the screen until it stops talking. It cannot be used.

We built T.O.M.M.Y. to solve this directly. Rather than shoving everything into one clunky Python file, we spread the workload across multiple CPU cores, doing their own thing and chatting via inter-process communication \cite{b4}. To get the actual controls working, we grabbed three entirely separate pieces of tech:
\begin{itemize}
    \item \textbf{Offline Voice Parsing}: We linked Porcupine \cite{b15} to listen for a wake trigger, but we dumped standard string mapping. Instead, we pipe the raw microphone feed into a deterministic spaCy parser to decode spoken syntax \cite{b23}.
...
\subsection{Nose Tracking over Eye Jitter}
Figuring out exactly where a human eyeball is aiming causes inherent algorithmic jitter \cite{b11}. Medical researchers use expensive infrared eye-bouncers to track gaze \cite{b7}. Trying to do that on a terrible $\$20$ laptop webcam means you have to decouple X/Y translation completely from the eyes. Instead, we rigidly bound spatial cursor translation to FaceMesh Node 1 (the physical Nose Tip). Nose spatial tracking natively absorbs micro-jitters without delay buffering. By measuring how far apart your top and bottom eyelids sit - the Eye Aspect Ratio - the computer can easily calculate whether you just blinked naturally or threw a heavy, intentional wink to trigger a click \cite{b21}. 

\subsection{Why Deterministic Syntax Engines Rule}
The old way of parsing speech involved heavy generative LLMs \cite{b13}. If you did not heavily constrain them, they failed or hallucinated. Nowadays, we have lightweight deterministic natural language processors (like spaCy) \cite{b23} that map logical intent directly inside your laptop's memory without ever requiring GPUs or cloud infrastructure. This was the holy grail for us. It meant our agent could extract grammatical actions from casual spoken commands at zero latency.

\subsection{The Universal Failure Point}
Combining speech and hand waving is not some brand-new idea; scientists have been writing theoretical papers about it since the 1980s \cite{b14}. Yet, nearly every single modern attempt crashes the second you take it out of a controlled lab \cite{b20}. The instant you try mashing heavy webcam arrays and heavy language tools inside one software loop, the computer processor completely chokes \cite{b22}.
...
\node (llm) [block,left=of planner] {NLP Engine};
...
Our fix was to abandon threading completely. We built a master loader called \texttt{tommy\_os.py} that uses the native \texttt{subprocess} library to launch the vision script on CPU Core 0 and the voice script on CPU Core 1 \cite{b17}. They do not share memory. To let them communicate, we built a tiny bridge file called \texttt{.tommy\_state.json}. If the voice agent needs to mute the mic, it just flips a boolean inside that text file. Two milliseconds later, the vision script reads the file and updates the desktop UI without ever dropping a single camera frame \cite{b6}.
...
\node (gaze) [block,below=of eye] {Nose Matrix};
...
\end{axis}
\end{tikzpicture}
\caption{Paradoxically, executing raw matrix math exclusively on the generic CPU (18 ms) vastly outperformed pushing data to the dedicated CUDA GPU (45 ms). This directly proves PCIe transfer overhead completely chokes extremely tiny 480p real-time camera arrays.}
\end{figure}

\subsection{Trashing Machine Learning for Geometry}
A ton of open-source clones use heavy image classifiers to detect a "thumbs up." You feed a neural net thousands of pictures, and it tries to guess the shape. It is incredibly sluggish and fails terribly in dark rooms \cite{b4}. We threw that idea out the window. We built an "Omni-Matrix Tool" that just runs pure linear math.
...
\end{tikzpicture}
\caption{The typical 21-point hand diagram. We strictly track the mathematical bounding distance between joint 4 and joint 8.}
\label{fig_hand_landmarks}
\end{figure}

\subsection{Damping Cursor Jitters}
Humans can't hold their hands perfectly still in mid-air. Mapping a fingertip directly to a 1080p canvas resulted in a mouse cursor that violently shook, making double-clicking anything impossible \cite{b22}. We silenced the shake by feeding the \texttt{PyAutoGUI} coordinate stream directly through an exponential moving average (EMA) algorithm \cite{b2}. After a lot of tweaking, tightening the alpha smoothing parameter down to $\alpha = 0.20$ generated lightning-fast UI translation \eqref{eq:ema}. Furthermore, humans cannot pinch without shifting their hand laterally due to tendon physics. Thus, we implemented a phase-locked ``Magnetic Cursor Lock''. By completely freezing the $X/Y$ buffer the exact millisecond the thumb and index finger pass a 60-pixel proximity limit, all organic clicking jitter is instantaneously eradicated natively prior to OS ingestion.

\begin{equation}
C_{coord_t} = (0.20 \times V_{raw}) + (0.80 \times C_{coord_{t-1}})
\label{eq:ema}
\end{equation}

\subsection{The Missing Thumb Bug}
We set up a gesture to toggle between hand tracking and nose/eye mode by checking if the user held up all 10 fingers. It was a disaster. Because regular webcams shoot straight-on, people's thumbs naturally hide behind the curve of their palms. The code would constantly drop the Boolean lock if the camera lost a thumb for a fraction of a second. We fixed this completely by mathematically dropping the gate requirement down to exactly 8 digits.

Once locked into gaze mode, standard boolean Eye Aspect Ratio math is inherently volatile \cite{b11}. Thus, we migrated the processor to a Time-Gated Ocular algorithm. Instead of analyzing spatial shapes, the loop tracks temporal durations: a rapid blink under $0.45s$ generates a strict Left Click; holding the blink to $2.0s$ fires a Right Click. Brutally, if a user forces a blind hold over $2.0$ seconds, the kernel detects it as a command override, instantly shutting down the ocular matrix map and rerouting machine resources actively back to Hand Tracking algorithms without vocal interruptions.
...
\section{Offline Deterministic Syntax Routing}

Using complex conversational NLP models created an incredibly annoying problem. Language models are desperately programmed to act like polite chatbots. A completely invisible backend OS loop cannot read a conversational paragraph; it needs strict programmatic arrays. To cleanly process syntax, we bypass generative networks entirely, migrating to the deterministic spaCy engine to quickly classify grammatical action trees locally.
...
\node [state, below of=listen, node distance=2.2cm] (think) {3: Syntactic Search (spaCy NLP)};
...
We chained the deterministic NLP directly to a hard-coded dictionary of Windows functions. Now, when the engine analyzes the audio string locally over the CPU, it just cross-references the dictionary and extracts parts-of-speech. Our backend code catches that dependency payload and jams it straight into the Windows \texttt{shell32} tool.

\subsection{Ambient Failsafes and Dynamic Listeners}
Relying entirely on free-tier third-party wake-word engines invites systemic collapse if API quotas expire. To protect the framework autonomously, we designed a self-healing fallback array. If the kernel catches a wake-agent blackout, it gracefully rewires the state to an ``Ambient Normal'' mode. Here, raw \texttt{SpeechRecognition} threads run uninterrupted and the script handles string slicing securely inside offline Python bounds. By interrogating a local JSON cache, users can verbally demand the OS to dynamically change its name (e.g., from ``Tommy'' to ``Jarvis'') on the fly, explicitly bypassing rigid neural net weight retraining entirely offline.
...
FaceMesh Nose & Iris Node Bounds & 92\% & 22 ms \\
Porcupine Wake Phoneme Verification & 98\% & 14 ms \\
Deterministic NLP Logic Routing Inference & 99\% & Constant ($<$ 15 ms)\\
\midrule
...
Since the microphone runner and the webcam runner do not actually share memory, throwing a fatal \texttt{SyntaxError} in the vision script completely detonates the camera feed, but the voice script stays perfectly alive and listening.
...
\STATE Spawn the OpenCV Mesh Engine isolated directly over CPU Core 0.
\STATE Trigger Porcupine acoustic arrays are isolated tightly inside CPU core 1.
...
\IF{Porcupine Mic-Energy heavily penetrates Acoustic Threshold Ceiling}
    \STATE Write \texttt{"True"} dynamically into the shared \texttt{JSON} IPC data payload file.
    \STATE Pause ambient voice-over playback routing.
    \STATE Shove the transcribed auditory string payload deep into the offline spaCy NLP.
...
Also, separating the memory heaps saved the project. Giving the NLP its own sandbox kept the camera feed completely shielded \cite{b5}. Granted, flat 2D depth projection still sucks—eventually, setups like this will need real stereoscopic or LIDAR hardware just to read Z-axis depth accurately without cheating the collision math.
...
The next obvious evolution for this is expanding the NLP syntactic trees. We want to wire up a complex deterministic state machine that holds conversational contexts indefinitely.

%--------------------------------------------

\section*{Acknowledgments}
We've got to give a massive shoutout to the crazy open-source engineering behind Google's MediaPipe. Without their core math, this wrapper would not exist. That being said, all the software plumbing, IPC bridging, self-healing Python loops, and chaotic geometry hacks documented in this paper were built entirely by us.
