    ...
    * **Eye Distance Tracking**: We track the exact pixel gap separating your upper and lower eyelids. This Eye Aspect Ratio [11] trick lets people click stuff on a screen just by winking deliberately.
    ...
    st.graphviz_chart('''
        digraph {
            ...
            "Webcam Input" -> "Gesture Detection"
            "Gesture Detection" -> "Gaze Estimation"
            "Gaze Estimation" -> "Gesture Classifier"
            "Microphone Input" -> "Speech Recognition"
            "Speech Recognition" -> "Wake Word Engine"
            "Gesture Classifier" -> "Task Planner"
            "Wake Word Engine" -> "Task Planner"
            "LLM Brain" -> "Task Planner"
            "Task Planner" -> "Multimodal Controller"
            "Multimodal Controller" -> "Automation Kernel"
            "Automation Kernel" -> "OS Actions"
        }
    ''')
    ...
    st.graphviz_chart('''
        digraph {
            node [shape=box, style=rounded]
            "User Entity" -> "Auditory Channel"
            "User Entity" -> "Spatial Input"
            "User Entity" -> "Ocular Trace"
            "Auditory Channel" -> "NLP Engine"
            "Spatial Input" -> "MediaPipe Hand"
            "Ocular Trace" -> "FaceMesh Iris"
            "NLP Engine" -> "IPC Master Controller State ('.json')"
            "MediaPipe Hand" -> "IPC Master Controller State ('.json')"
            "FaceMesh Iris" -> "IPC Master Controller State ('.json')"
            "IPC Master Controller State ('.json')" -> "Background WMI / PyAutoGUI"
            "Background WMI / PyAutoGUI" -> "Windows Host OS"
        }
    ''')
